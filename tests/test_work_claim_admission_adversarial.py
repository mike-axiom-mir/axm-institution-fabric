...
class WorkClaimAdmissionAdversarialTests(unittest.TestCase):
    """Lane 03 continuity attacks for the bounded work-claim admission slice.

    These witnesses distinguish an exact/schema-valid occupancy object from an
    occupancy whose own entry-base lane relationship was ever grounded. A generic
    immutable object-store write must not silently substitute for the canonical
    occupancy-admission transition when a later lifecycle step treats the occupancy
    as operational.
    """
    ...
    def test_adv_033_a_missing_occupancy_entry_base_cannot_open_claim(self) -> None:
        ...
        The occupancy object is schema-valid, exact, active, and names the same
        logical lane as the claim. Its entry base is nevertheless only a syntactic
        reference and was never persisted. Canonical ``admit_occupancy`` would reject
        this object. ``open_work_claim`` must not turn generic object persistence into
        a substitute admission proof.
        """
    ...
    def test_adv_033_b_ambiguous_occupancy_entry_lane_cannot_open_claim(self) -> None:
        ...
        Two exact lane versions with one logical id make the occupancy's own entry
        relation ambiguous. A separate claim base contains exactly one lane and would
        otherwise let the claim resolve cleanly. The later transition must preserve
        the earlier relationship failure instead of laundering it through a newer
        exact base.
        """
