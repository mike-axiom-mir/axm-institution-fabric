...
def typed_artifact(
    ...
    *,
    ...
    return {
        ...
        "version": version,
        "content_ref": "tests/test_source_live_observation_adversarial_verify_error.py",
        "provenance": {
...
class SourceLiveObservationVerifyErrorAdversarialTests(unittest.TestCase):
    def test_adv_058_j_caller_verify_error_cannot_forge_context_local_absence(self):
        """Caller-owned verifier errors must not manufacture a live-context not-found fact.

        ADV-058-F established that a successful caller-owned ``_verify_existing`` shadow
        cannot by itself prove ``exact_observed``; the bounded repair now requires the
        base verifier to run afterward. ADV-058-I established the matching rule at the
        outer ``load_bytes`` layer for caller-forged not-found errors. This adjacent
        fixture checks the still-caller-visible verifier layer on that same exact-store
        observation path.

        The exact target is genuinely stored in the supplied ``FilesystemObjectStore``.
        The store type, root, ``objects_dir``, ``load_bytes``, ``_object_path``, and
        ``schema_dir`` remain unchanged. Only the exact instance's ``_verify_existing``
        shadow raises a forged ``ObjectNotFoundError`` before the function-owned base
        verifier can read and reproduce the exact immutable identity.
        ...
        with tempfile.TemporaryDirectory() as source_tmp:
            ...
            def forged_not_found(reference: str, schema_name: str):
                ...
                raise ObjectNotFoundError("caller-forged verifier absence")
            ...
            with mock.patch.object(
                ...
                try:
                    ...
                    self.assertEqual(
                        ...
                        "caller-shadowed _verify_existing error must not forge context-local absence",
                    )
